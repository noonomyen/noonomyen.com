...
type AsyncCompilePromiseConstructor = Promise<[{ [key: string]: string; }, string, CheerioAPI]>;
type CompilePromiseConstructor = Promise<[{ [key: string]: string }, string, CheerioAPI]>;
type CompileResults = [{ [key: string]: string; }, string, CheerioAPI];
...
const setupBuildDir = (workspace: string) => {
    const PWD = cwd();
    chdir(workspace);
...
const TSCompile = (RELEASE: boolean, workspace: string): Promise<{ [key: string]: string }> => {
...
const TailwindCompile = (RELEASE: boolean, workspace: string, wait = false): string | Promise<string> => {
    if (wait) {
...
const HTMLLoader = (workspace: string) => {
...
const AsyncCompile = (RELEASE: boolean, workspace: string): AsyncCompilePromiseConstructor => {
    return Promise.all([
...
const Compile = (RELEASE: boolean, workspace: string): CompilePromiseConstructor => {
    return Promise.all([
        TSCompile(RELEASE, workspace),
        HTMLLoader(workspace),
    ]).then((results) => {
...
const ERROR_PAGE = (RELEASE: boolean, workspace: string) => {
...
if (argv.indexOf("ts-wait") != -1) {
    setupBuildDir(workspace);
    Compile(RELEASE, workspace)
        .then(Target.ERROR_PAGE(RELEASE, workspace))
        .then(end)
        .catch(error);
} else {
    setupBuildDir(workspace);
    AsyncCompile(RELEASE, workspace)
        .then(Target.ERROR_PAGE(RELEASE, workspace))
        .then(end)
        .catch(error);
}
